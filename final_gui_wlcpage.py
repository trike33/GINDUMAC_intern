...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QGridLayout, QGraphicsDropShadowEffect, QSizePolicy, QTextEdit, QDialog
)
...
N'hésitez pas à me contacter si vous avez besoin d'informations supplémentaires."""
    ...
    def init_ui(self):
        ...
        self.warning_label = QLabel("⚠️ <span style='color: orange;'>Please double check the generated template!</span>")
        self.warning_label.setTextFormat(Qt.RichText)  # Enable HTML formatting
    ...
    def parse_input(self, text, lang):
        patterns = {
            "name": {
                "en": r"(Dear|Hello|Hi)\s+([\w\s\-\.’'éèáàçñüöäëïŚśŻżŹźĄąĆćĘęŁłŃńÓóŚś]+?)[,\n]",
                "it": r"(Gentile|Ciao)\s+([\w\s\-\.’'éèáàçñüöäëï]+?)[,\n]",
                "fr": r"(Bonjour|Cher)\s+([\w\s\-\.’'éèáàçñüöäëï]+?)[,\n]"
            },
        ...
        return (
            ...
            self.clean_spaces(machine.group(1).replace(".", "")) if machine else "[Insert machine name]",
    ...
    def process_text(self):
        ...
        # Detect [Insert ...] placeholders in the result
        if any("[Insert" in field for field in [client_name, machine_name, location, link]):
            missing_fields = []
            if "[Insert" in client_name:
                missing_fields.append("client name")
            if "[Insert" in machine_name:
                missing_fields.append("machine name")
            if "[Insert" in location:
            ...
            self.update_status(
                f"⚠️ Some fields were not parsed correctly: {readable_fields.capitalize()}.\nPlease check the pasted message.",
                color="red"
            )
            return
...
class EmailSentTab(QWidget):
    ...
    def update_status(self, message):
        ...
        elif "No URLs" in message:
            self.status_label.setStyleSheet("color: orange;")
        else:
            self.status_label.setStyleSheet("") # This reverts to the main theme color
...
class SellerFollowupTab(QWidget):
    ...
    def init_ui(self):
        main_layout = QVBoxLayout(self)
        self.warning_label = QLabel("⚠️ <span style='color: orange;'>Please remember to check the name and lang!</span>")
        self.warning_label.setTextFormat(Qt.RichText)  # Enable HTML formatting
        self.warning_label.setStyleSheet("font-weight: bold;")
        main_layout.addWidget(self.warning_label)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        logo_frame = QFrame()
        # The header color is kept constant for branding
        ...
        self.theme_button = QPushButton() # Text will be set by apply_stylesheet
        ...
        for name, TabClass in tabs.items():
                    # Pass the main_window reference specifically to ActionsTab
    ...
    def apply_stylesheet(self, dark_mode=False):
        if dark_mode:
            # New Dark Mode Palette: Dark Grey & Turquoise
            bg_color = "#2E2E2E"
            pane_bg = "#3C3C3C"
            border_color = "#555555"
            tab_bg = "#4A4A4A"
            ...
            groupbox_bg = "#2E2E2E"
            button_bg = "#00C7B1"         # Turquoise
            button_hover = "#00A997"
            button_pressed = "#008B7D"
            button_disabled_bg = "#454545"
            ...
            text_color = "#003366"
            text_color_selected = "#003366"
            groupbox_bg = "#f0f0f0"
            button_bg = "#0078D7"        # Original Blue
            button_hover = "#005A9E"
        ...
        # Update the theme button's text and icon
        self.theme_button.setText("☀️ Light Mode" if dark_mode else "🌙 Dark Mode")
        ...
        self.setStyleSheet(f"""
            ...
            QProgressBar {{
                ...
                background-color: {progress_bg}; color: {button_bg};
            }}
    ...
    def closeEvent(self, event):
        ...
        It ensures all background threads from all tabs are stopped gracefully.
        """
        print("Closing application. Stopping all background threads...")
        for i in range(self.notebook.count()):
            tab = self.notebook.widget(i)
            
            if hasattr(tab, 'stop_thread'):
                print(f"Stopping thread for tab: {self.notebook.tabText(i)}")
                tab.stop_thread()
            elif hasattr(tab, 'reset_process'):
                print(f"Resetting process for tab: {self.notebook.tabText(i)}")
                tab.reset_process()
...
if __name__ == "__main__":
    ...
    if login.exec_() == QDialog.Accepted:
        ...
        main_window.show()
        main_window.open_welcome_window()  # Show welcome after login
        sys.exit(app.exec_())
    else:
        sys.exit(0)  # Exit if login cancelled or failed